...
> Литиллм/Ollama — внешний сервер, настраивается только через `.env`. Контейнеры не нужны.
...
## Фаза 2 — Chat Sessions + история контекста
...
Логика: вопрос → embedding → cosine similarity с сохранёнными векторами вопросов в Redis.
HIT (≥ порога) → готовый ответ (~200мс). MISS → полный RAG → сохранить в Redis.

Redis структура (из архитектуры):
...
- [x] `docker/docker-compose.yml` — раскомментировать/убрать `ollama` (не нужен — внешний сервер)
